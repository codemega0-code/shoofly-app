...
const CONFIRMABLE_DELIVERY_STATES = new Set(['OUT_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED']);
...
export async function confirmDelivery(params: {
  ...
  if (request.status !== 'ORDER_PAID_PENDING_DELIVERY') {
    throw new Error(`Request is in status ${request.status}, cannot confirm delivery`);
  }
  ...
  if (!lastDeliveryStatus || !CONFIRMABLE_DELIVERY_STATES.has(lastDeliveryStatus)) {
    logger.warn('qr.confirm.invalid_delivery_state', {
      requestId,
      clientId,
      deliveryStatus: lastDeliveryStatus || null,
    });
    throw new Error(`Delivery is not in confirmable state (${lastDeliveryStatus || 'none'})`);
  }
